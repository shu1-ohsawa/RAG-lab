...
1. **リポジトリのクローン**
    ```bash
    git clone https://github.com/shu1-ohsawa/RAG-lab.git
    cd RAG-lab
    ```
...
2. **Ollama のセットアップ**
    ...
    公式サイト[ollama.com](https://ollama.com/ "ollama") から利用するOSにあったもの（本開発環境はWindows版）をダウンロードしてインストールします。


3. **LLMのダウンロード**


    PowerShellを開き、以下のコマンドを打って Gemma 2 (2b) をダウンロード＆起動します（軽量なモデルなので普通のノートPCでもサクサク動ため、2bを想定）。

    ```bash
    ollama run gemma2:2b
    ```

    初回はダウンロードが走ります。終わるとチャットが始まりますが、確認できたら /bye で終了してOKです。


    [余談]モデルがダウンロードされるディレクトリ

    デフォルトでは、Cドライブのユーザーフォルダの奥深くに保存されます。

    ```bash
    デフォルトパス: C:\Users\<ユーザー名>\.ollama\models
    ```
